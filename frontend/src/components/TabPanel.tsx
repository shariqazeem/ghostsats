  ...
  return (
    <div className="glass-card overflow-hidden">
      ...
      <div className="p-1.5 mx-6 mt-6 bg-[var(--bg-secondary)] rounded-xl flex relative">
        <motion.div
          className="absolute top-1.5 bottom-1.5 w-[calc(50%-6px)] bg-white rounded-lg shadow-[var(--shadow-sm)]"
          animate={{ x: tab === "shield" ? 6 : "calc(100% + 6px)" }}
          transition={{ type: "spring", stiffness: 400, damping: 30 }}
        />
        <button
          onClick={() => setTab("shield")}
          className={`relative z-10 flex-1 flex items-center justify-center gap-2 py-2.5 text-[13px] font-semibold transition-colors cursor-pointer ${
