...
| Contract | Address |
|----------|---------|
| ShieldedPool | `0x026c4c09ae0b87f2986a288d122eba93d36aa66e3936f13294f7735ee7996c44` |
...
**Explorer:** https://sepolia.starkscan.co/contract/0x026c4c09ae0b87f2986a288d122eba93d36aa66e3936f13294f7735ee7996c44
...
**What to verify:**
- Phase shows "Generating zero-knowledge proof..." (ZK commitment/nullifier computed)
...
Withdrawal flow:
  Browser → generateWithdrawalProof(secret, blinder, denomination)
    ├── zkNullifier = Poseidon(secret, 1)  [computed client-side]
    └── proof = [] (verifier=zero) / garaga_calldata (production)
  → withdraw_private(denomination, zk_nullifier, zk_commitment, proof, merkle_path, ...)
    ├── Looks up pedersen_commitment from zk_commitment
    ├── Verifies ZK proof (if verifier != zero)
...
1. **ZK proof verification skipped** — The on-chain verifier address is set to zero, so `withdraw_private` doesn't verify the actual ZK proof. The Garaga verifier is built and tested (passes on Sepolia fork), but deploying it requires matching Cairo editions. The contract is production-ready — just set the verifier address.

2. **Proof generation is client-side only** — In production, the full pipeline would be: `noir_js → bb.js WASM → garaga calldata formatting`. For the hackathon, we compute commitments/nullifiers with `poseidon-lite` and pass an empty proof.

3. **Relayer requires manual startup** — No hosted relayer yet. Run `npm run relayer` locally for gasless withdrawals.
